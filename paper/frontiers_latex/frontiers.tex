...
\end{center}
 \textbf{\refstepcounter{figure}\label{fig:br_violin} Figure \arabic{figure}.}{ A) Runtimes distribution with outliers B) Cost distribution box plots, Across bid ratios for processing 1,000 datasets in C-PAC }
\end{figure}
...
\end{center}
 \textbf{\refstepcounter{figure}\label{fig:av_violin} Figure \arabic{figure}.}{ C) Runtimes distribution with outliers D) Cost distribution box plots, Across availability zones for processing 1,000 datasets in C-PAC }
\end{figure}


%%% Availability zone
\subsection{Choosing an availability zone}

There are three factors in determining the optimal availability zone for computation: price history, costs related to non-computation (e.g. storage, data transfer), and the geographic region from where data will be uploaded and downloaded from the cloud.

In order to achieve the lowest computation cost for processing a dataset, the spot history for the availability of interest should have as stable and low of a market price as possible. Often, this combination is hard to find. Some availability zones may have a low mean or median price history but are also volatile. In these cases, the computation costs are minimized to a degree, however the computation time tends to be much greater. Likewise, availability zones with higher spot prices but less volatile activity minimize computation time but increase the associated costs. Figure \ref{fig:spot_history}

% Spot history
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10cm]{../../spot-model/plots/spot_histories.pdf}
\end{center}
 \textbf{\refstepcounter{figure}\label{fig:spot_history} Figure \arabic{figure}.}{ The spot prices from mid March early September 2015 for the Linux/UNIX c3.8xlarge instance } 
\end{figure}

\subsection{Choosing a bid price}
